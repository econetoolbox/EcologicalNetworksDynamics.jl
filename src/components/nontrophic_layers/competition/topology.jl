...
function F.late_check(raw, bp::Raw)
    ...
    @check_template A P "potential competition link"
end

F.expand!(raw, bp::Raw) = expand!(raw, bp.A)
expand!(raw, A) = expand_topology!(raw, :competition, A)
...
function F.late_check(raw, bp::Random)
    ...
    Lmax = @get raw.competition.potential_links.number
    (; L) = bp
    if !isnothing(L)
        s(n) = n > 1 ? "s" : ""
        L > Lmax && checkfails("Cannot draw L = $L competition link$(s(L)) \
                                with only $np producer$(s(np)) (max: L = $Lmax).")
    end
end

function F.expand!(raw, bp::Random)
    A = random_links(raw, bp, Internals.potential_competition_links)
    expand!(raw, A)
end
...
@expose_data edges begin
    property(competition.links.matrix)
    get(CompetitionLinks{Bool}, sparse, "competition link")
    ref(raw -> raw._scratch[:competition_links])
    @species_index
    depends(Topology)
end
