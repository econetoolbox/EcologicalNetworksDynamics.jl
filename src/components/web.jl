    ...
    blueprints.eval(
        quote
            ...
            F.expand!(raw, bp::Adjacency, _, model) =
                reflexive_web_adjacency_expand!(ew, raw, bp.A, model::Model)
...
function reflexive_web_matrix_late_check(ew::EdgeWeb, A::AbstractMatrix, model::Model)
    a, b = size(A)
...
function reflexive_web_adjacency_late_check(
    ew::EdgeWeb,
    ::Network,
    A::Adjacency,
    model::Model,
    ...
    @check_list_refs A "$p link" index
    A
end

function reflexive_web_adjacency_expand!(ew::EdgeWeb, raw::Network, A, model::Model)
    class = sourcename(ew)
    index = getproperty(model, class)._index
