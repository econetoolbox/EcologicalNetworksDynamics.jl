...
module Calls
    ...
    using ..Conflicts: Value

    using EcologicalNetworksDynamics.Framework:
        Framework, Blueprint, Component, System, define_blueprint, define_conflicts

    using EcologicalNetworksDynamics.Tests: @test_err, addfails, @conflfails
    using Test
    ...
    @testset "Declaring components conflicts." begin
        ...
        @test_err(define_conflicts(A),
            """
            In conflicts definition:
            At least two components are required to declare a conflict not only $_A.
            """)
        ...
        confs(C) = sort(
            collect(
                Iterators.map(Framework.all_conflicts(typeof(C))) do (a, b, r)
...
module Abstracts
    ...
    @testset "Abstract component conflicts semantics." begin
        ...
        addfails.@sysconflict(S(D.b(), H.b()), _H, nothing, [H_b], _D, A, "H dislikes A.")
        # (without explicit reason)
        addfails.@sysconflict(S(H.b(), D.b()), _D, A, [D_b], _H, nothing, nothing)
        ...
        # (with an explicit reason)
        addfails.@sysconflict(S(D.b(), E.b()), _E, C, [E_b], _D, B, "C dislikes B.")
        # (without explicit reason)
        addfails.@sysconflict(S(E.b(), D.b()), _D, B, [D_b], _E, C, nothing,)
